...
#4-5번 강우이벤트 중 최대 강수량은, 비가 연속으로 올 때, 하나의 강우
    print(f"최대 강우량은 {max_rainfall_event(rainfall):.1f}mm입니다.")
#4-6번 가장 더운날 top 3(tmax의 최댓값 3개)
    print(f"가장 더운 날 top3는 {top_rank(tmax, 3)}mm입니다.") 
#5-1번 여름철 6, 7, 8월 총강수량 - sumifs(rainfall, months, selected=[6,7,8])
    print(f"여름철(6월-8월) 총 강수량은 {sumifs(rainfall, months,[6,7,8]):.1f}mm 입니다.")
#5-2번 2021, 2022년 총 강수량(이 문제는 2001-2022년 자료 활용)
